...
// Initialize the numerical output format
const std::string IOFormatter::numericalOutputFormat = "0.##############";

std::string IOFormatter::formatErrorMessage(int line, int positionInLine, const std::string& message) {
    std::ostringstream oss;
    oss << "Error at [" << line << ", " << positionInLine << "]: " << message;
    return oss.str();
}
