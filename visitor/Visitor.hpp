...
class Visitor : public SASBaseVisitor<DapValue> {
private:
    std::istream& stdin;
    std::ostream& stdout;
    DapInterpreterState& state;
...
public:
    // Constructor
    Visitor(std::istream& in, std::ostream& out, DapInterpreterState& s) : stdin(in), stdout(out), state(s), printStream(std::cout), dataTable(Table()) {}
    ...
    virtual DapValue visitParse(SASParser::ParseContext *ctx) override {
        return visitChildren(ctx);
    }

    // Implement other visit methods similarly...

    virtual DapValue visitData_main(SASParser::Data_mainContext *ctx) override {
        // Implementation for visiting data_main context
        // You can handle this as per your requirements
        return visitChildren(ctx);
    }

    virtual DapValue visitData_stmt(SASParser::Data_stmtContext *ctx) override {
        // Implementation for visiting data_stmt context
        // You can handle this as per your requirements
        std::string dataStepName = ctx->getText();
        dataTable = Table::create(dataStepName);
        return DapValue(dataTable); // Assuming DapValue constructor accepts a Table object
    }

    virtual DapValue visitInput_stmt(SASParser::Input_stmtContext *ctx) override {
        ...
        // Return a placeholder DapValue or handle it according to your logic
        return DapValue(ctx); // Modify this based on your requirements
    }

    // Implement the remaining visit methods...

    virtual DapValue visitPut_stmt(SASParser::Put_stmtContext *ctx) override {
        // Implementation for visiting put_stmt context
        // You can handle this as per your requirements
        return visitChildren(ctx);
    }

    virtual DapValue visitDatastmt_cmd(SASParser::Datastmt_cmdContext *ctx) override {
    ...
    virtual DapValue visitMeans_main(SASParser::Means_mainContext *ctx) override {
        // Implementation for visiting means_main context
        // You can handle this as per your requirements
        return visitChildren(ctx);
    }

    virtual DapValue visitMeans_proc(SASParser::Means_procContext *ctx) override {
        // Implementation for visiting means_proc context
        // You can handle this as per your requirements
        return visitChildren(ctx);
    }

    virtual DapValue visitStatistic_keyword(SASParser::Statistic_keywordContext *ctx) override {
    ...
    virtual DapValue visitRun_main(SASParser::Run_mainContext *ctx) override {
        // Implementation for visiting run_main context
        // You can handle this as per your requirements
        return visitChildren(ctx);
    }

    virtual DapValue visitRun_stmt(SASParser::Run_stmtContext *ctx) override {
